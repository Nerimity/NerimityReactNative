      ...
      <ScrollView keyboardShouldPersistTaps="handled" style={{ ...styles.scrollViewContainer, maxHeight: height / 2 }}>
        <View style={styles.modalContainer}>
          <View style={styles.titleContainer}>
            {props.icon && <Icon name={props.icon} size={24} color={props.color || Colors.primaryColor} />}
            <Text style={{...styles.title, color: props.color || Colors.primaryColor}}>{props.title}</Text>
          </View>
          {props.children}
        </View>
      </ScrollView>
