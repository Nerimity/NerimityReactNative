...
export const MessageContextMenu = observer((props: { close: () => void, serverId?: string; message: RawMessage }) => {
  ...
  const onDeleteClick = () => {
      ...
      createPortal(close => (
        <DeleteMessageModal close={close} message={props.message} serverId={props.serverId} />
  ...
  const items: ContextMenuOption[] = [
    ...
    ...(showDelete() ? [{ icon: 'delete', title: 'Delete Message', color: Colors.alertColor, onPress: onDeleteClick }] : []),
...
const DeleteMessageModal = (props: {close: () => void; message: RawMessage, serverId?: string}) => {
  ...
  return (
    <Modal title="Delete Message" icon="delete" close={props.close} color={Colors.alertColor}>
      <Text style={{marginLeft: 5}}>Would you like to delete this message?</Text>
      <MessageItem preview item={props.message} />
      <View style={{flexDirection: 'row', marginTop: 10}}>
        <CustomButton flex={1} title="Don't Delete" color={Colors.primaryColor} onPress={props.close}  icon="close" />
        <CustomButton flex={1} title="Delete" color={Colors.alertColor} onPress={onDeleteClick} icon="delete" />
      </View>
    </Modal>
  )
}